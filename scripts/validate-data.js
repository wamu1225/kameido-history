...
// 4) 内部リンク [/kameido-history/PATH/] の PATH が実在する記事ID（または about/privacy）であること
import fs from 'fs';
...
const expected = ['name-origin', 'history', 'katori', 'tenjin', 'daikon', 'kuzumochi', 'ukiyoe', 'spots', 'faq'];
...
// 定義済み figure キー（figures-data.ts の FIGURE_DATA から）
...
for (const { id, block } of blocks) {
  ...
  // 内部リンクの宛先が実在するか
  for (const lk of [...content.matchAll(/\]\(\/kameido-history\/([a-z0-9-]+)\//g)]) {
    const target = lk[1];
